...
$C_UID =   $_SESSION["UID"];
// 先選擇a_user中的Love欄位
$sql = "SELECT a_user.Love, c_movie.Image FROM a_user,c_movie WHERE  UID = '$C_UID'";
	...
	for ($i = 0; $i < count($mydata); $i++) {
		switch ($mydata[$i]) {
			case 1:
				...
				$sql = "SELECT c_movie.Moviename, c_movie.Image FROM c_movie WHERE c_movie.Love = '$mydata[$i]'";
				$result = execute_sql($conn, "funmovie", $sql);
				//從資料庫抓片名，存到$row
